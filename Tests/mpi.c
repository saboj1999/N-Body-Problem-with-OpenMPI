...
To Run:
mpiexec -n 2 ./mpi.o <number of bodies> <number of steps>

For process count higher than 2 on my machine (my cpu has 2 cores):
mpiexec --oversubscribe -n 4 ./mpi.o <number of bodies> <number of steps>
...
int main(int argc, char **argv) {
    ...
    Body *allBodies = malloc(numBodies * sizeof(Body));  // Ensure this is correct
    ...
    if(world_rank == 0)
    {
        ...
        printf("Final positions of bodies:\n");
        for (int i = 0; i < numBodies; i++) {
            printf("Body %d: x = %f, y = %f, z = %f\n", i, allBodies[i].x, allBodies[i].y, allBodies[i].z);
        }
    }
